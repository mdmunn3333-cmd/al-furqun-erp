...
(function(){
  ...
  loginForm?.addEventListener('submit',async(e)=>{
    ...
    if(error){
      loginError.textContent = error.message.includes('Invalid login credentials') ? 'Email অথবা password ভুল।' : error.message;
      loginBtn.disabled=false;loginBtn.textContent='Sign In';
      return;
    }
    // SIGNED_IN will trigger the cloud-load handler above. Do not call
    // loadCloud here or the auth/database requests can race each other.
  ...
  // IMPORTANT: Do not await database/network work inside onAuthStateChange.
  // Supabase Auth holds an internal lock during auth events; awaiting a second
  // Supabase request here can leave the UI stuck on the login screen.
  client.auth.onAuthStateChange((event,session)=>{
    ...
    if(event==='SIGNED_IN'){
      ...
      // Run cloud loading after the auth callback has returned.
      setTimeout(async()=>{
        const ok=await loadCloud(session.user);
        if(ok) hideLogin();
      },0);
    }
  });

  // Initial auth check. This is kept separate from the auth-event callback
  // so an existing session is loaded exactly once.
  client.auth.getSession().then(({data})=>{
    if(data?.session){
      updateProfile(data.session.user);
      setTimeout(async()=>{
        const ok=await loadCloud(data.session.user);
        if(ok) hideLogin();
      },0);
    }else{
      showLogin('');
    ...
    console.error(err);
    showLogin('Authentication connection failed.');
  });
})();
